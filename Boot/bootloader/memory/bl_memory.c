...
bl_status_t bl_memory_mark_application_valid(uint32_t app_size, const uint8_t *signature, uint16_t signature_length)
{
    return bl_flash_mark_app_valid(app_size, signature, signature_length);
}
