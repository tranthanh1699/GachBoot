...
def generate_fls_code(config: Dict, output_dir: str) -> List[str]:
    ...
    generated_files = [header_path, source_path]
    
    # Generate Flashblock configuration if present
    flashblock_config = config.get('flashblock_config')
    if flashblock_config:
        flashblock_gen_dir = os.path.join(output_dir, 'Flashblock_Gen')
        os.makedirs(flashblock_gen_dir, exist_ok=True)
        
        flashblock_header_content = generate_flashblock_config(flashblock_config)
        flashblock_header_path = os.path.join(flashblock_gen_dir, 'Flashblock_PBCfg.h')
        with open(flashblock_header_path, 'w', encoding='utf-8') as f:
            f.write(flashblock_header_content)
        
        generated_files.append(flashblock_header_path)
...
def generate_flashblock_config(config_data: Dict) -> str:
    """
    Generate Flashblock_PBCfg.h from flashblock_config
    
    Args:
        config_data: flashblock_config section from JSON
    
    Returns:
        Generated header content
    """
    # Extract config with defaults
    queue_size = config_data.get('queue_size', 8)
    max_data_size = config_data.get('max_data_size', 256)
    write_chunk_size = config_data.get('write_chunk_size', 32)
    erase_timeout = config_data.get('erase_timeout_ms', 2000)
    write_timeout = config_data.get('write_timeout_ms', 100)
    auto_verify = config_data.get('auto_verify', False)
    description = config_data.get('description', 'Flash Block Configuration')
    
    # Calculate RAM usage
    ram_usage = queue_size * (40 + max_data_size)
 ...
 * 
 * Current: {queue_size} operations
 * RAM usage: ~{ram_usage} bytes ({queue_size} * {40 + max_data_size} bytes per entry)
 */
#define FLASHBLOCK_CFG_QUEUE_SIZE          {queue_size}u
 ...
 * 
 * Current: {max_data_size} bytes
 * Constraint: Must be multiple of FLASHBLOCK_CFG_WRITE_CHUNK_SIZE ({write_chunk_size})
 */
#define FLASHBLOCK_CFG_MAX_DATA_SIZE       {max_data_size}u
 ...
 * 
 * Current: {write_chunk_size} bytes
 * 
 * @note CRITICAL: Must match hardware capability!
 */
#define FLASHBLOCK_CFG_WRITE_CHUNK_SIZE    {write_chunk_size}u
 ...
 * 
 * Current: {erase_timeout}ms
 */
#define FLASHBLOCK_CFG_ERASE_TIMEOUT_MS    {erase_timeout}u
 ...
 * 
 * Current: {write_timeout}ms
 */
#define FLASHBLOCK_CFG_WRITE_TIMEOUT_MS    {write_timeout}u
 ...
 * 
 * Current: {1 if auto_verify else 0} ({'Enabled' if auto_verify else 'Disabled'})
 * 0 = Disabled (faster, less safe)
 * 1 = Enabled (slower, more reliable)
 */
#define FLASHBLOCK_CFG_AUTO_VERIFY         {1 if auto_verify else 0}u
 ...
 */
#define FLASHBLOCK_QUEUE_RAM_USAGE_BYTES   {ram_usage}u
